...
export default function Profile() {
  ...
  const handleFileUpload = (file) => {
    ...
    const fileName = new Date().getTime() + file.name; //inorder to have a unique file name, if someone uploads 2 times one file user cant upload it, so to prevent it we written this
    const storageRef = ref(storage, fileName);
    const uploadTask = uploadBytesResumable(storageRef, file); //to see % of upload done

    //to track changes
    uploadTask.on(
      'state_changed',
      (snapshot) => { //piece of info from each state change
        const progress =
